<template>
  <div>
    <loading v-if="categorias == undefined"/>
    <div v-else>
      <div class="selector">
     ...
     <lister porMarca="no" :size="8":idQuery="currentSlide.id" :key="currentIndex"/>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    currentSlide: function() {
      return this.categorias[Math.abs(this.currentIndex) % this.categorias.length];
    }
  },
