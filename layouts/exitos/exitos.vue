<template>
    <div id="service-list">
      ...
      <nuxt-link class="service-content" :to="'/casos-de-exito/'+ exito.slug" v-for="exito in exitos">
        <div class="image">
          <img :src="exito.featured_image_url">
        </div>
        <div class="text">
          <div class="title">
          <h2><strong>{{exito.title.rendered}}</strong></h2>
          </div>
          <p v-html="exito.excerpt.rendered"></p>
        </div>
      </nuxt-link>
    </div>
...
export default {
  ...
  mounted: async function() {
      try {
        var Exitos = await this.$axios.get('https://blog.mechatronik-group.com/wp-json/wp/v2/posts?categories=43');
        this.exitos = Exitos.data;
        this.isLoading = false;
